...
func NewABACPolicy(treeChecker TreeChecker, ownerID string, identity *crypto.Identity) *ABACPolicy {
	return &ABACPolicy{
...
func (p *ABACPolicy) SetIdentity(identity *crypto.Identity) {
	p.identity = identity
}

func (p *ABACPolicy) Identity() *crypto.Identity {
	return p.identity
}
