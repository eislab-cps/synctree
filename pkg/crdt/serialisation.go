...
func (c *TreeCRDT) SecureImportJSON(rawJSON []byte, identity *crypto.Identity) (core.NodeID, error) {
...
func (c *TreeCRDT) SecureImportJSONToMap(rawJSON []byte, parentID core.NodeID, key string, identity *crypto.Identity) (core.NodeID, error) {
	if parentID == "" {
		if c.Root == nil {
			return "", errors.New("cannot import JSON without a root node")
		}
...
func (c *TreeCRDT) SecureImportJSONToArray(rawJSON []byte, parentID core.NodeID, identity *crypto.Identity) (core.NodeID, error) {
	if parentID == "" {
		if c.Root == nil {
			return "", errors.New("cannot import JSON without a root node")
		}
...
func (c *TreeCRDT) importJSON(rawJSON []byte, parentID core.NodeID, edgeLabel string, idx int, nodeType core.NodeType, clientID core.ClientID, secure bool, identity *crypto.Identity) (core.NodeID, error) {
	version := 1
	var parent *NodeCRDT
	if parentID == "" {
		parent = c.Root
...
func (c *TreeCRDT) secureImportRecursive(v interface{}, parent *NodeCRDT, edgeLabel string, idx int, nodeType core.NodeType, clientID core.ClientID, secure bool, identity *crypto.Identity) (core.NodeID, error) {
	version := 1
...
func (c *TreeCRDT) Load(data []byte) error {
	...
	for idStr, val := range nodesRaw {
		...
		node := &NodeCRDT{
			...
			LiteralValue: nodeMap["literalValue"],
